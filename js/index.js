...
const weatherInformationsContainer = document.getElementById(
...
const ApiKey = "4ce829ae32c54bd1845230725242009";
...
setTimeout(function () {
  ...
  document
    .getElementById("hoursContainer")
    .classList.add("border-top", "border-secondary", "border-2");
  document
    .querySelector(".search-days-col")
    .classList.add("border-start", "border-secondary", "border-2");
}, 3100);
      ...
      navigator.geolocation.getCurrentPosition(
        async function (position) {
          const currentLocation = await fetch(
            `https://api.weatherapi.com/v1/search.json?key=${ApiKey}&&q=${position.coords.latitude},${position.coords.longitude}`
          );
...
async function search(location = "alex") {
  ...
  const response = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=${ApiKey}&q=${location}&days=6&hour`
  );
...
function mainChanges(data) {
  mainTitleContainer.innerHTML = `
    <h1 id="mainLocationName" class="location op">${
      data.location.name + `, ` + data.location.country
    }</h1>
    <p class="today op"><span class="day">${
      days[
        new Date(
          data.location.localtime.replace("-", "/").replace("-", "/")
        ).getDay()
      ]
    }, </span><span class="date">${
    months[date.getMonth(data.location.localtime.split(" ")[0])]
  } ${date.getDate(
    data.location.localtime.split(" ")[0]
  )} ${date.getFullYear()} , </span><span class="time">${
    data.location.localtime.split(" ")[1]
  }</span></p>`;

  weatherInformationsContainer.innerHTML = `
            <h3 id="mainLocationDeg" class="deg display-3 fw-bold op">${Math.round(
              data.current.temp_c
            )}<sup>o</sup>C</h3>
            <div class="weather-condition text-center op">
                <img class="weather-condition-icon op" src=${
                  data.current.condition.icon
                }  alt="">
                <p class="weather-condition-text fw-bolder op">${
                  data.current.condition.text
                }</p>
            </div>
            <ul class="weather-datils m-0 d-flex flex-column justify-content-between h-100 p-0 op">
                <li class="pressure op"><i class="fa-solid fa-temperature-three-quarters me-1"></i>pressure ${
                  data.current.pressure_mb
                }mb</li>
                <li class="humidity op"><i class="fa-solid fa-droplet me-1"></i>humidity ${
                  data.current.humidity
                }%</li>
                <li class="wind op"><i class="fa-solid fa-wind me-1"></i>wind ${
  ...
  for (let i = 0; i < timesContainer.length; i++) {
    timesContainer[i].innerHTML = `
        ...
        <img class="status" src=${
          data.forecast.forecastday[0].hour[i + 12].condition.icon
        } width="40px" height="40px" alt="">
        <p class="m-0 deg fw-bold">${Math.round(
          data.forecast.forecastday[0].hour[i + 12].temp_c
        )}<sup>o</sup>C</p>`;
  }
}
...
async function displayHoursCardsDatiles(intendedHour) {
  let response = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=${ApiKey}&q=${mainLocationName.innerHTML}&days=6&hour`
  );

  let data = await response.json();

  mainTitleContainer.innerHTML = `
    <h1 id="mainLocationName" class="location">${
      data.location.name + `, ` + data.location.country
    }</h1>
    <p class="today s"><span class="day">${
      days[
        new Date(
          data.location.localtime.replace("-", "/").replace("-", "/")
        ).getDay()
    ...
    data.location.localtime.split(" ")[0]
  )} ${date.getFullYear()} , </span><span class="time-now">${
    data.forecast.forecastday[0].hour[intendedHour].time.split(" ")[1]
  }</span></p>`;

  weatherInformationsContainer.innerHTML = `
            <h3 id="mainLocationDeg" class="deg display-3 fw-bold s">${Math.round(
              data.forecast.forecastday[0].hour[intendedHour].temp_c
            )}<sup>o</sup>C</h3>
            <div class="weather-condition text-center s">
                <img class="weather-condition-icon s" src=${
                  data.forecast.forecastday[0].hour[intendedHour].condition.icon
                }  alt="">
                <p class="weather-condition-text fw-bolder s">${
                  data.forecast.forecastday[0].hour[intendedHour].condition.text
                }</p>
            </div>
            <ul class="weather-datils m-0 d-flex flex-column justify-content-between h-100 p-0 s">
                <li class="pressure s"><i class="fa-solid fa-temperature-three-quarters me-1"></i>pressure ${
                  data.forecast.forecastday[0].hour[intendedHour].pressure_mb
                }mb</li>
                <li class="humidity s"><i class="fa-solid fa-droplet me-1"></i>humidity ${
                  data.forecast.forecastday[0].hour[intendedHour].humidity
                }%</li>
                <li class="wind s"><i class="fa-solid fa-wind me-1"></i>wind ${
                  data.forecast.forecastday[0].hour[intendedHour].wind_kph
                } kph</li>
            </ul>`;
...
function hoursBackgroundChanger(data, intendedHour) {
  if (
    data.forecast.forecastday[0].hour[intendedHour].condition.text.includes(
      "Clear"
    ) &&
    data.forecast.forecastday[0].hour[intendedHour].condition.icon.includes(
      "night"
    )
  ) {
    header.style.cssText = `
...
function daysForecast(data) {
  for (let i = 0; i < daysContainer.length; i++) {
    daysContainer[i].innerHTML = `
        <img class="status-icon bg-black bg-opacity-25 rounded p-1" src=${
          data.forecast.forecastday[i + 1].day.condition.icon
        } width="40px" height="40px" alt="">
                <div class="date-day d-flex flex-column">
                    <p class="date m-0"><span class="day-name">${
                      days[
                        new Date(
          ...
          data.forecast.forecastday[i + 1].date
            ...
            .replace("-", "/")
    ...
    } ${data.forecast.forecastday[i + 1].date.substr(8)}</span></p>
                ...
                <div class="deg d-flex flex-column border-start border-2 ps-3">
                    <p class="min-deg m-0">${Math.round(
                      data.forecast.forecastday[i + 1].day.mintemp_c
                    )}<sup>o</sup></p>
                    <p class="max-deg m-0">${Math.round(
...
async function displayDaysDatiles(intendedDay) {
  ...
  mainTitleContainer.innerHTML = `
    ...
    <p class="today s"><span class="day">${
      days[
        new Date(
          data.forecast.forecastday[intendedDay].date
            .replace("-", "/")
            .replace("-", "/")
        ).getDay()
      ]
  ...
  } ${data.forecast.forecastday[intendedDay].date.substr(
    8
  )} , </span><span class="time-now">12:00</span></p>`;

  weatherInformationsContainer.innerHTML = `
            <h3 id="mainLocationDeg" class="deg display-3 fw-bold s">${Math.round(
              data.forecast.forecastday[intendedDay].day.maxtemp_c
            )}<sup>o</sup>C</h3>
            <div class="weather-condition text-center s">
                <img class="weather-condition-icon s" src=${
                  data.forecast.forecastday[intendedDay].day.condition.icon
                }  alt="">
                <p class="weather-condition-text fw-bolder s">${
                  data.forecast.forecastday[intendedDay].day.condition.text
                }</p>
            </div>
            <ul class="weather-datils m-0 d-flex flex-column justify-content-between h-100 p-0 s">
                <li class="pressure"><i class="fa-solid fa-temperature-three-quarters me-1"></i>pressure ${
                  data.forecast.forecastday[intendedDay].hour[intendedDay]
                    .pressure_mb
                }mb</li>
                <li class="humidity"><i class="fa-solid fa-droplet me-1"></i>humidity ${
                  data.forecast.forecastday[intendedDay].day.avghumidity
                }%</li>
                <li class="wind"><i class="fa-solid fa-wind me-1"></i>wind ${
                  data.forecast.forecastday[intendedDay].day.maxwind_kph
                } kph</li>
            </ul>`;
...
function daysBackgroundChanger(data, intendedDay) {
  if (
    data.forecast.forecastday[intendedDay].day.condition.text.includes(
      "Clear"
    ) &&
    data.forecast.forecastday[intendedDay].day.condition.icon.includes("night")
  ) {
    header.style.cssText = `
            background-image: linear-gradient(45deg, rgba(0, 0, 0, 0.750), rgba(0, 0, 0, 0.750)), url(./media/imgs/night/clear.jpg);`;
    container.style.cssText = `
    ...
    data.forecast.forecastday[intendedDay].day.condition.icon.includes("day")
  ) {
    header.style.cssText = `
            background-image: linear-gradient(45deg, rgba(0, 0, 0, 0.750), rgba(0, 0, 0, 0.750)), url(./media/imgs/day/sunny-clear.jpg);`;
    container.style.cssText = `
    ...
    data.forecast.forecastday[intendedDay].day.condition.text.includes("Fog") &&
    data.forecast.forecastday[intendedDay].day.condition.icon.includes("night")
  ) {
    header.style.cssText = `
            background-image: linear-gradient(45deg, rgba(0, 0, 0, 0.750), rgba(0, 0, 0, 0.750)), url(./media/imgs/night/mist-night.jpg);`;
